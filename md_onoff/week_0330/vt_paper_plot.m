...
vNt_ed_pzav = repmat(mean(mean(vNt_ed_ll,3),1),[180,1,1,1]);

vNt_ed_ptzav = repmat(mean(mean(vNt_ed_pav,1),4),[180,1,1,720]);

vNt_ed_var = squeeze( mean ( mean( (vNt_ed_pav - vNt_ed_ptzav).^2 ,1) ,4) );
vNt_ed_sdev = sqrt(vNt_ed_var);

vNt_ed_var_zav = squeeze( mean ( mean( (vNt_ed_pzav - vNt_ed_ptzav).^2 ,1) ,4) );
vNt_ed_sdev_zav = sqrt(vNt_ed_var_zav);
...
vNt_ed_pzav_tneg = repmat(mean(mean(vNt_ll_tneg,3),1),[180,1,1,1]);
vNt_ed_pzav_tpos = repmat(mean(mean(vNt_ll_tpos,3),1),[180,1,1,1]);

vNt_ed_ptzav_tneg = repmat(mean(mean(vNt_ed_pav_tneg,1),4),[180,1,1,720]);
vNt_ed_ptzav_tpos = repmat(mean(mean(vNt_ed_pav_tpos,1),4),[180,1,1,720]);

vNt_ed_var_tneg = squeeze( mean ( mean( (vNt_ed_pav_tneg - vNt_ed_ptzav_tneg).^2 ,1) ,4) );
vNt_ed_sdev_tneg = sqrt(vNt_ed_var_tneg);
vNt_ed_var_zav_tneg = squeeze( mean ( mean( (vNt_ed_pzav_tneg - vNt_ed_ptzav_tneg).^2 ,1) ,4) );
vNt_ed_sdev_zav_tneg = sqrt(vNt_ed_var_zav_tneg);

vNt_ed_var_tpos = squeeze( mean ( mean( (vNt_ed_pav_tpos - vNt_ed_ptzav_tpos).^2 ,1) ,4) );
vNt_ed_sdev_tpos = sqrt(vNt_ed_var_tpos);
vNt_ed_var_zav_tpos = squeeze( mean ( mean( (vNt_ed_pzav_tpos - vNt_ed_ptzav_tpos).^2 ,1) ,4) );
vNt_ed_sdev_zav_tpos = sqrt(vNt_ed_var_zav_tpos);
...
save('/project/rg312/mat_files/vt_paper_plot.mat','vNt_ll_tzav_000','vNt_ll_tzav_tneg_000','vNt_ll_tzav_tpos_000')
