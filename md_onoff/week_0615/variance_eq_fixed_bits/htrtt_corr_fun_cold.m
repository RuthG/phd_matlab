function [corr,htrtt_ed,th_sd,htrt_sd] = htrtt_corr_fun_cold(htrt,th)
...
th_ed_mask_neg = +(th_ed_ll < 0);

htrt_sd = sqrt(squeeze(mean(mean(th_ed_mask_neg.*htrt_ed_ll.^2,1),4)));
th_sd = sqrt(squeeze(mean(mean(th_ed_mask_neg.*th_ed_ll.^2,1),4)));
htrtt_ed = squeeze(mean(mean(th_ed_mask_neg.*htrt_ed_ll.*th_ed_ll,1),4));
