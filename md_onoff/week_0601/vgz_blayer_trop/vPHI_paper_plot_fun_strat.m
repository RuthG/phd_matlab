...
function [vPHI_ed_tzav,vPHI_ed_tzav_neg,vPHI_ed_tzav_pos,vPHI_ed_sdev,vPHI_ed_sdev_neg,vPHI_ed_sdev_pos] = vPHI_paper_plot_fun_strat(run)
...
vNPHI_ed_pav = mean(vNPHI_ed_ll(:,:,tlev(xn)+1:25,:),3);
...
vNPHI_ed_pav_tneg = squeeze(mean(vNPHI_ll_tneg(:,:,tlev(xn)+1:25,:) ,3));
vNPHI_ed_pav_tpos = squeeze(mean(vNPHI_ll_tpos(:,:,tlev(xn)+1:25,:) ,3));
