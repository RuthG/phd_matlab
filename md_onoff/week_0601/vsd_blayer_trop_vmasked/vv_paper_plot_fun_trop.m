...
function [vv_ed_tzav,vv_ed_tzav_neg,vv_ed_tzav_pos,vv_ed_sdev,vv_ed_sdev_neg,vv_ed_sdev_pos] = vv_paper_plot_fun_trop(run)
...
vNvN_ed_pav = mean(vNvN_ed_ll(:,:,blev(xn)+1:tlev(xn),:),3);
...
vNvN_ll_tneg = vNvN_ed_ll.*vN_ed_mask_neg;
vNvN_ll_tpos = vNvN_ed_ll.*vN_ed_mask_pos;

vNvN_ed_pav_tneg = squeeze(mean(vNvN_ll_tneg(:,:,blev(xn)+1:tlev(xn),:) ,3));
vNvN_ed_pav_tpos = squeeze(mean(vNvN_ll_tpos(:,:,blev(xn)+1:tlev(xn),:) ,3));
