...
function [vv_ed_tzav,vv_ed_tzav_neg,vv_ed_tzav_pos,vv_ed_sdev,vv_ed_sdev_neg,vv_ed_sdev_pos] = vv_paper_plot_fun_troplid(run)
...
vNvN_ed_pav = mean(vNvN_ed_ll(:,:,1:tlev(xn),:),3);
...
vNvN_ed_pav_tneg = squeeze(mean(vNvN_ll_tneg(:,:,1:tlev(xn),:) ,3));
vNvN_ed_pav_tpos = squeeze(mean(vNvN_ll_tpos(:,:,1:tlev(xn),:) ,3));
