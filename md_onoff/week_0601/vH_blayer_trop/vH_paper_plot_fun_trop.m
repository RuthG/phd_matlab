...
function [vH_ed_tzav,vH_ed_tzav_neg,vH_ed_tzav_pos,vH_ed_sdev,vH_ed_sdev_neg,vH_ed_sdev_pos] = vH_paper_plot_fun_trop(run)
...
vNH_ed_pav = mean(vNH_ed_ll(:,:,blev(xn)+1:tlev(xn),:),3);
...
vNH_ed_pav_tneg = squeeze(mean(vNH_ll_tneg(:,:,blev(xn)+1:tlev(xn),:) ,3));
vNH_ed_pav_tpos = squeeze(mean(vNH_ll_tpos(:,:,blev(xn)+1:tlev(xn),:) ,3));
