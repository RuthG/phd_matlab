...
function t_ac = t_autocorr_l13_fun(run)
...
norm = (mean(t_ed_ll(90,xn,13,:).^2,4) + mean(t_ed_ll(90,xs,13,:).^2,4))./2 ;


for k=0:10
for j=1:720-k
t_ed_h1(:,:,j,11-k) = t_ed_ll(90,xn,13,j+k).*t_ed_ll(:,:,13,j);
t_ed_h2(:,:,j,11-k) = t_ed_ll(90,xs,13,j+k).*t_ed_ll(:,:,13,j);
end
end
...
for k=1:10
for j=1:720-k
t_ed_h1(:,:,j,k+11) = t_ed_ll(90,xn,13,j).*t_ed_ll(:,:,13,j+k);
t_ed_h2(:,:,j,k+11) = t_ed_ll(90,xs,13,j).*t_ed_ll(:,:,13,j+k);
end
end
...
save(['/project/rg312/mat_files/t_ac_l13_' run '.mat'],'t_ac')
