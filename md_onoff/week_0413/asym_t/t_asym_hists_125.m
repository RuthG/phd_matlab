...
t_125_ll = squeeze(cube2latlon(xc,yc,t_125(:,:,5,1:719),xi,yi));
t_125_ll_tav = repmat(mean(t_125_ll,3),[1 1 719]);
t_ed_125_ll = t_125_ll - t_125_ll_tav;
clear('t_125')

t_125_skew = skewness(reshape(t_ed_125_ll,[numel(t_ed_125_ll),1]));
...
t_ed_125_counts = hist(reshape(t_ed_125_ll,[numel(t_ed_125_ll),1]),xbins);
t_ed_125_counts_cos(i,:) = t_ed_125_counts.*cos(yi(i).*pi./180);

end

t_hist = sum(t_ed_125_counts_cos,1)./sum(sum(t_ed_125_counts_cos));
...
save('/project/rg312/mat_files/t_hists_asym_125.mat')
