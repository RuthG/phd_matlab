...
t_150_ll = squeeze(cube2latlon(xc,yc,t_150(:,:,5,1:719),xi,yi));
t_150_ll_tav = repmat(mean(t_150_ll,3),[1 1 719]);
t_ed_150_ll = t_150_ll - t_150_ll_tav;
clear('t_150')

t_150_skew = skewness(reshape(t_ed_150_ll,[numel(t_ed_150_ll),1]));
...
t_ed_150_counts = hist(reshape(t_ed_150_ll,[numel(t_ed_150_ll),1]),xbins);
t_ed_150_counts_cos(i,:) = t_ed_150_counts.*cos(yi(i).*pi./180);

end

t_hist = sum(t_ed_150_counts_cos,1)./sum(sum(t_ed_150_counts_cos));
...
save('/project/rg312/mat_files/t_hists_asym_150.mat')
