...
htrt_025_skew = skewness(reshape(htrt_ed_025_ll,[numel(htrt_ed_025_ll),1]));
...
htrt_ed_025_counts = hist(reshape(htrt_ed_025_ll,[numel(htrt_ed_025_ll),1]),xbins);
htrt_ed_025_counts_cos(i,:) = htrt_ed_025_counts.*cos(yi(i).*pi./180);

end

htrt_hist = sum(htrt_ed_025_counts_cos,1)./sum(sum(htrt_ed_025_counts_cos));
...
save('/project/rg312/mat_files/htrt_hists_asym_025.mat')
