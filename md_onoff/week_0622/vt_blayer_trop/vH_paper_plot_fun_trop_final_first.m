...
function [vNH_ed_out ,vNH_ed_warm_out ,vNH_ed_cold_out ,vNH_ed_serr ,vNH_ed_warm_serr ,vNH_ed_cold_serr] = vH_paper_plot_fun_trop_final_first(run)
...
vNH_ed_pzav(:,:,year) = squeeze(mean(mean(vNH_ed(:,:,1:tlev(xn),:),1),3));
vNH_ed_warm_pzav(:,:,year) = squeeze(mean(mean(vNH_ed_warm(:,:,1:tlev(xn),:),1),3));
vNH_ed_cold_pzav(:,:,year) = squeeze(mean(mean(vNH_ed_cold(:,:,1:tlev(xn),:),1),3));
