...
function [vNH_ed_out ,vNH_ed_warm_out ,vNH_ed_cold_out ,vNH_ed_serr ,vNH_ed_warm_serr ,vNH_ed_cold_serr] = vH_paper_plot_fun_trop_final_last(run)
...
vNH_ed_pzav(:,:,year-5) = squeeze(mean(mean(vNH_ed(:,:,1:tlev(xn),:),1),3));
vNH_ed_warm_pzav(:,:,year-5) = squeeze(mean(mean(vNH_ed_warm(:,:,1:tlev(xn),:),1),3));
vNH_ed_cold_pzav(:,:,year-5) = squeeze(mean(mean(vNH_ed_cold(:,:,1:tlev(xn),:),1),3));
...
vNH_ed_serr = sqrt(vNH_ed_var)./sqrt(360.*2.*5);
vNH_ed_warm_serr = sqrt(vNH_ed_warm_var)./sqrt(360.*2.*5);
vNH_ed_cold_serr = sqrt(vNH_ed_cold_var)./sqrt(360.*2.*5);
