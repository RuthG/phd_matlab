...
function [vNt_ed_out ,vNt_ed_warm_out ,vNt_ed_cold_out ,vNt_ed_serr ,vNt_ed_warm_serr ,vNt_ed_cold_serr] = vt_paper_plot_fun_blayer_final(run)
...
vNt_ed_pzav(:,:,year) = squeeze(mean(mean(vNt_ed(:,:,1:blev(xn),:),1),3));
vNt_ed_warm_pzav(:,:,year) = squeeze(mean(mean(vNt_ed_warm(:,:,1:blev(xn),:),1),3));
vNt_ed_cold_pzav(:,:,year) = squeeze(mean(mean(vNt_ed_cold(:,:,1:blev(xn),:),1),3));
