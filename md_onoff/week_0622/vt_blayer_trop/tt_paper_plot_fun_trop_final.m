...
function [tt_ed_out ,tt_ed_warm_out ,tt_ed_cold_out ,tt_ed_serr ,tt_ed_warm_serr ,tt_ed_cold_serr] = tt_paper_plot_fun_trop_final(run)
...
tt_ed_pzav(:,:,year) = squeeze(mean(mean(tt_ed(:,:,1:tlev(xn),:),1),3));
tt_ed_warm_pzav(:,:,year) = squeeze(mean(mean(tt_ed_warm(:,:,1:tlev(xn),:),1),3));
tt_ed_cold_pzav(:,:,year) = squeeze(mean(mean(tt_ed_cold(:,:,1:tlev(xn),:),1),3));
