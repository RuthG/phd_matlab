...
function [vv_ed_out ,vv_ed_warm_out ,vv_ed_cold_out ,vv_ed_serr ,vv_ed_warm_serr ,vv_ed_cold_serr] = vv_paper_plot_fun_blayer_final(run)
...
vv_ed_warm = vv_ed.*t_mask_pos;
vv_ed_cold = vv_ed.*t_mask_neg;

vv_ed_pzav(:,:,year) = squeeze(mean(mean(vv_ed(:,:,1:blev(xn),:),1),3));
vv_ed_warm_pzav(:,:,year) = squeeze(mean(mean(vv_ed_warm(:,:,1:blev(xn),:),1),3));
vv_ed_cold_pzav(:,:,year) = squeeze(mean(mean(vv_ed_cold(:,:,1:blev(xn),:),1),3));
