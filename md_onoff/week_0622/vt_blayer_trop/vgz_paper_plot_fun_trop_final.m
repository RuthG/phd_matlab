...
function [vNph_ed_out ,vNph_ed_warm_out ,vNph_ed_cold_out ,vNph_ed_serr ,vNph_ed_warm_serr ,vNph_ed_cold_serr] = vgz_paper_plot_fun_trop_final(run)
...
vNph_ed_pzav(:,:,year) = squeeze(mean(mean(vNph_ed(:,:,1:tlev(xn),:),1),3));
vNph_ed_warm_pzav(:,:,year) = squeeze(mean(mean(vNph_ed_warm(:,:,1:tlev(xn),:),1),3));
vNph_ed_cold_pzav(:,:,year) = squeeze(mean(mean(vNph_ed_cold(:,:,1:tlev(xn),:),1),3));
