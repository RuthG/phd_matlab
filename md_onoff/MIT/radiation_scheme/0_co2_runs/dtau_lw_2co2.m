...
n=25;
  ...
  for k= 1:n
    del_tau(:,k) = (ir_tau_co2 + ir_tau_wv.*q_in(1:45,k).^0.5).*(p_half(k+1) - p_half(k))./p_half(n+1);
    dtrans(:,k) = exp(-del_tau(:,k));
    del_tau_win(:,k) = (ir_tau_co2_win + ir_tau_wv_win1.*q_in(1:45,k) + ir_tau_wv_win2.*q_in(1:45,k).^2).*(p_half(k+1) - p_half(k))./p_half(n+1);
    dtrans_win(:,k) = exp(-del_tau_win(:,k));

    del_tau_np(:,k) = (ir_tau_co2_np + ir_tau_wv_np.*q_in(1:45,k).^0.5).*(p_half(k+1) - p_half(k))./p_half(n+1);
...
plot(q_in(1:45,:).*1000,dtau_dn,'xk')
hold on
plot(q_in(1:45,:).*1000,del_tau,'c')
set(gca,'FontSize',10)
ylim([ 0 7])
...
plot(q_in(1:45,:).*1000,dtau_win_dn,'xk')
hold on
plot(q_in(1:45,:).*1000,del_tau_win,'c')
set(gca,'FontSize',10)
ylim([0 0.35])
xlabel('Specific Humidity, g/kg','FontSize',12)
...
for k=1:25
dtrans_dn(:,k) = (fxdn_lw(1:45,k+1) - B(:,k)) ./ (fxdn_lw(1:45,k) - B(:,k));
dtrans_win_dn(:,k) = (fxdn_win(1:45,k+1) - B_win(:,k)) ./ (fxdn_win(1:45,k) - B_win(:,k));
end
...
axes('position',[pos1x pos1y xSize_sub ySize_sub])
plot(q_in(1:45,:).*1000,(dtau_dn - dtau_dn_1co2)./0.04,'xk')
set(gca,'FontSize',10)
ylim([-5 5])
xlabel('Specific Humidity, g/kg','FontSize',12)
...
axes('position',[pos2x pos2y xSize_sub ySize_sub])
plot(q_in(1:45,:).*1000,(dtau_win_dn - dtau_win_dn_1co2)./0.04,'xk')
set(gca,'FontSize',10)
ylim([-0.25 0.25])
xlabel('Specific Humidity, g/kg','FontSize',12)
