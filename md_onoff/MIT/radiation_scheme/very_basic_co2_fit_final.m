...
dtau_win_dn_20co2 = -log(dtrans_win_dn);
...
tau_diff_mean(5) = mean(mean((real(dtau_dn_20co2(2:45,1:5) - dtau_dn_1co2(2:45,1:5)))))./0.04;

tau_win_diff_mean(1) = 0;
tau_win_diff_mean(2) = mean(mean((real(dtau_win_dn_2co2(2:89,:)  - dtau_win_dn_1co2(2:89,:)))))./0.04;
tau_win_diff_mean(3) = mean(mean((real(dtau_win_dn_4co2(2:89,:)  - dtau_win_dn_1co2(2:89,:)))))./0.04;
tau_win_diff_mean(4) = mean(mean((real(dtau_win_dn_10co2(2:45,:) - dtau_win_dn_1co2(2:45,:)))))./0.04;
tau_win_diff_mean(5) = mean(mean((real(dtau_win_dn_20co2(2:45,:) - dtau_win_dn_1co2(2:45,:)))))./0.04;
...
print('-dpng','basic_co2_fit_nonwin_final.png')
...
print('-dpng','basic_co2_fit_win_final.png')

co2_const = b_l.*log(co2./360);
co2_win_const = b_l_win.*log(co2./360) %b_q_win(1).*(co2./360 - 1).^2 + b_q_win(2).*(co2./360 - 1);
...
del_tau_20 = del_tau + co2_const(5).*0.04; %tau_diff_mean(4).*0.04;
...
del_tau_win_20 = del_tau_win + co2_win_const(5).*0.04; %tau_win_diff_mean(4).*0.04;
...
print('-dpdf','dtau_co2fit_2_final.pdf')
...
print('-dpdf','dtau_co2fit_4_final.pdf')
...
print('-dpdf','dtau_co2fit_10_final.pdf')
...
axes('position',[pos1x pos1y xSize_sub ySize_sub])
plot(q_in(1:45,:).*1000,dtau_dn_20co2,'xk')
hold on
plot(q_in.*1000,del_tau_20,'c')
set(gca,'FontSize',10)
ylim([ 0 7])
xlabel('Specific Humidity, g/kg','FontSize',12)
ylabel('d\tau','FontSize',12)
...
axes('position',[pos2x pos2y xSize_sub ySize_sub])
plot(q_in(1:45,:).*1000,dtau_win_dn_20co2,'xk')
hold on
plot(q_in.*1000,del_tau_win_20,'c')
set(gca,'FontSize',10)
ylim([0 0.35])
xlabel('Specific Humidity, g/kg','FontSize',12)
...
print('-dpdf','dtau_co2fit_20_final.pdf')