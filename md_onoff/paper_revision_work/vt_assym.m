...
function [vNt_ed_w, vNt_ed_c, tt_ed_w, tt_ed_c] = vt_assym(run)
    ...
    for year = 1:10
        ...
        vNt_ed_warm(:,:,year) = squeeze(mean(mean( vNt_ed.*t_mask_pos,4),1));
        vNt_ed_cold(:,:,year) = squeeze(mean(mean( vNt_ed.*t_mask_neg,4),1));
        tt_ed_warm(:,:,year) = squeeze(mean(mean( tt_ed.*t_mask_pos,4),1));
        tt_ed_cold(:,:,year) = squeeze(mean(mean( tt_ed.*t_mask_neg,4),1));
                
    end

    vNt_ed_w = mean(vNt_ed_warm,3);
    vNt_ed_c = mean(vNt_ed_cold,3);
    tt_ed_w = mean(tt_ed_warm,3);
    tt_ed_c = mean(tt_ed_cold,3);
