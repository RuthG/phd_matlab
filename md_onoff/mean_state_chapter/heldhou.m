...
theta_eq_th(:,k) = t_eq_th(:,k)./convthetatoT(k,1);
...
teq_th_zav = T_th_zav + L./cp .* q_th_zav;
...
thetaeq_th_zav(:,j)=teq_th_zav(:,j)./convthetatoT(j,1);
end
...
%deltheta_eq_th = theta_eq_th(45,:) - theta_eq_th(89,:);
...
theta_eq_th_vav = mean(theta_eq_th,2);
...
deltheta_eq(5) = theta_eq_th_vav(45) - theta_eq_th_vav(89);
...
psi_width(5) = yi(45 + min(find(psi_th_hzav(:,13)<=0)));
...
trop_th(i) = rC(min(find((dTdz_th(i,:) >= -2))));
...
trop_height_th(i) = height_th(i,(min(find((dTdz_th(i,:) >= -2)))));
...
bulk_stab_th(i) = theta_th_zav(i,min(find((dTdz_th(i,:) >= -2)))) - theta_th_zav(i,1);
...
T0_th = mean(theta_eq_th_vav(2:89));
...
T0_th_max = max(theta_eq_th_vav(2:89));
...
T0_th_min = min(theta_eq_th_vav(2:89));
...
phi_hh(5) = sqrt(consts .* trop_height_th(45) .* deltheta_eq(5) ./ T0_th).*180./pi;
...
phi_hh_gen(5) = sqrt(consts .* trop_height_th(45) .* deltheta_eq(5) ./ T0).*180./pi;
...
phi_hh_max(5) = sqrt(consts .* trop_height_th(45) .* deltheta_eq(5) ./ T0_th_max).*180./pi;
...
phi_hh_min (5) = sqrt(consts .* trop_height_th(45) .* deltheta_eq(5) ./ T0_th_min ).*180./pi;
...
bulk_stab_th_weighted(i) = bulk_stab_th(i).*cosphi(i);
...
trop_height_th_weighted(i) = trop_height_th(i).*cosphi(i);
...
bulk_stab_q_av = sum(bulk_stab_q_weighted(55:65))./sum(cosphi(55:65));
bulk_stab_d_av = sum(bulk_stab_d_weighted(55:65))./sum(cosphi(55:65));
bulk_stab_th_av = sum(bulk_stab_th_weighted(55:65))./sum(cosphi(55:65));
...
trop_height_th_av = sum(trop_height_th_weighted(55:65))./sum(cosphi(55:65));
...
phi_h(5) = (bulk_stab_th_av .* trop_height_th_av).^(0.25);
...
phi_h_alt(5) = (bulk_stab_th(45) .* trop_height_th(45)).^(0.25);
...
[dthetadp_th,moo] = gradient(theta_th_zav,-4000);
...
[dthetaeqdp_th,moo] = gradient(thetaeq_th_zav,-4000);
...
delta_v_th(:,i) = -2.* dthetadp_th(1:89,i).*(98000 - trop_th)';
...
Sc_th(:,i) = -f(1:89)'./ beta(1:89)' .*dthetady_th(1:89,i) ./ delta_v_th(1:89,i);
...
deltaeq_v_th(:,i) = -2.* dthetaeqdp_th(1:89,i).*(98000 - trop_th)';
...
Sc_th_eq(:,i) = -f(1:89)'./ beta(1:89)' .*dthetaeqdy_th(1:89,i) ./ deltaeq_v_th(1:89,i);
end
...
Sc_th_hzav(i,:) = (Sc_th(45+i,:) + Sc_th(46-i,:))./2;
end
...
phi_s(5) = yi(45 + min(find(Sc_th_hzav(:,1)>=1)));
