...
load(['/project/rg312/mat_files/snapshot_data_final/run_th/vN_th_' num2str(modelyear) '.mat'])
vNll=cube2latlon(xc,yc,vN_th,xi,yi);

for i=1:90
integrand(:,i,:,:) = vNll(:,i,:,:).*rad.*cos(yi(i)*dtr)./g.*4000.*deltalam;
end

load(['/project/rg312/mat_files/snapshot_data_final/run_th/theta_th_' num2str(modelyear) '.mat'])
thetall=cube2latlon(xc,yc,theta_th,xi,yi);
...
save('/project/rg312/mat_files/chapter2_variables/psi_theta_th.mat','psi_mean')
