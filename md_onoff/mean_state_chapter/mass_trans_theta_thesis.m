...
load(['/project/rg312/mat_files/snapshot_data_final/run_100/vN_100_' num2str(modelyear) '.mat'])
vNll=cube2latlon(xc,yc,vN_100,xi,yi);

for i=1:90
integrand(:,i,:,:) = vNll(:,i,:,:).*rad.*cos(yi(i)*dtr)./g.*4000.*deltalam;
end

load(['/project/rg312/mat_files/snapshot_data_final/run_100/theta_100_' num2str(modelyear) '.mat'])
thetall=cube2latlon(xc,yc,theta_100,xi,yi);
...
%define a mask that identifies positions where theta is in a given range
  mask=+(thetall>=theta_ranges(j) & thetall<theta_ranges(j+1));
...
save('/project/rg312/mat_files/chapter2_variables/psi_theta_100.mat','psi_mean')