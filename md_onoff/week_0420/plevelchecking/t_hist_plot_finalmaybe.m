...
vN_000_ll = squeeze(cube2latlon(xc,yc,vN_000(:,:,10,1:719),xi,yi));
...
vN_100_ll = squeeze(cube2latlon(xc,yc,vN_100(:,:,10,1:719),xi,yi));
...
t_ed_000_counts = hist(reshape(t_ed_000_ll,[numel(t_ed_000_ll),1]),xbins_t);
t_ed_000_counts_cos(i,:) = t_ed_000_counts.*cos(yi(i).*pi./180);

t_ed_100_counts = hist(reshape(t_ed_100_ll,[numel(t_ed_100_ll),1]),xbins_t);
t_ed_100_counts_cos(i,:) = t_ed_100_counts.*cos(yi(i).*pi./180);


vN_ed_000_counts = hist(reshape(vN_ed_000_ll,[numel(vN_ed_000_ll),1]),xbins_v);
vN_ed_000_counts_cos(i,:) = vN_ed_000_counts.*cos(yi(i).*pi./180);

vN_ed_100_counts = hist(reshape(vN_ed_100_ll,[numel(vN_ed_100_ll),1]),xbins_v);
...
print('-dpng','t_hist_plot_l10.png')
...
wetleg = ['Wet'];
legend(dryleg, wetleg)
print('-dpng','v_hist_plot_l10.png')
