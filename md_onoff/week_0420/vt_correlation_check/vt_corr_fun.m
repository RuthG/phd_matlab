...
vN_cs = vN_cs.(v);
vN_ll = squeeze(cube2latlon(xc,yc,vN_cs(:,:,5,1:719),xi,yi));
...
vN_sd = sqrt(squeeze(mean(mean(vN_ed_ll.^2,1),3)));
t_sd = sqrt(squeeze(mean(mean(t_ed_ll.^2,1),3)));
vt_ed = squeeze(mean(mean(vN_ed_ll.*t_ed_ll,1),3));
