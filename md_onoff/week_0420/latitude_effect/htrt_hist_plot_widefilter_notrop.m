...
%create mask so we only look at T' greater than 2, less that -2
...
htrt_ed_000_pos_in = htrt_ed_000_pos(:,91-i,:);
htrt_ed_000_pos_in(htrt_ed_000_pos_in == 0) =[];
htrt_ed_000_neg_in = htrt_ed_000_neg(:,91-i,:);
htrt_ed_000_neg_in(htrt_ed_000_neg_in == 0) =[];

htrt_ed_100_pos_in = htrt_ed_100_pos(:,91-i,:);
htrt_ed_100_pos_in(htrt_ed_100_pos_in == 0) =[];
htrt_ed_100_neg_in = htrt_ed_100_neg(:,91-i,:);
htrt_ed_100_neg_in(htrt_ed_100_neg_in == 0) =[];

%bin data at each latitude and weight bin contribution by area (i.e. cos(lat))
htrt_ed_000_counts = hist(reshape(htrt_ed_000_ll(:,91-i,:),[numel(htrt_ed_000_ll(:,91-i,:)),1]),xbins);
htrt_ed_000_counts_cos(i,:) = htrt_ed_000_counts_cos(i,:) + htrt_ed_000_counts.*cos(yi(i).*pi./180);

htrt_ed_000_pos_counts = hist(reshape(htrt_ed_000_pos_in,[numel(htrt_ed_000_pos_in),1]),xbins);
htrt_ed_000_pos_counts_cos(i,:) = htrt_ed_000_pos_counts_cos(i,:) + htrt_ed_000_pos_counts.*cos(yi(i).*pi./180);

htrt_ed_000_neg_counts = hist(reshape(htrt_ed_000_neg_in,[numel(htrt_ed_000_neg_in),1]),xbins);
htrt_ed_000_neg_counts_cos(i,:) = htrt_ed_000_neg_counts_cos(i,:) + htrt_ed_000_neg_counts.*cos(yi(i).*pi./180);


htrt_ed_100_counts = hist(reshape(htrt_ed_100_ll(:,91-i,:),[numel(htrt_ed_100_ll(:,91-i,:)),1]),xbins);
htrt_ed_100_counts_cos(i,:) = htrt_ed_100_counts_cos(i,:) + htrt_ed_100_counts.*cos(yi(i).*pi./180);

htrt_ed_100_pos_counts = hist(reshape(htrt_ed_100_pos_in,[numel(htrt_ed_100_pos_in),1]),xbins);
htrt_ed_100_pos_counts_cos(i,:) = htrt_ed_100_pos_counts_cos(i,:) + htrt_ed_100_pos_counts.*cos(yi(i).*pi./180);

htrt_ed_100_neg_counts = hist(reshape(htrt_ed_100_neg_in,[numel(htrt_ed_100_neg_in),1]),xbins);
htrt_ed_100_neg_counts_cos(i,:) = htrt_ed_100_neg_counts_cos(i,:) + htrt_ed_100_neg_counts.*cos(yi(i).*pi./180);
...
print('-dpng','htrt_hist_plot_dry_notrop.png')
...
print('-dpng','htrt_hist_plot_wet_notrop.png')
...
print('-dpng','htrt_hist_pos_plot_wide_notrop.png')
...
legend(dryleg, wetleg)
print('-dpng','htrt_hist_neg_plot_wide_notrop.png')
