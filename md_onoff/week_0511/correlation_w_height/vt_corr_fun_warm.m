function [corr,vt_ed,t_sd,vN_sd] = vt_corr_fun_warm(v,t)
...
vN_sd = sqrt(squeeze(mean(mean(t_ed_mask_pos.*vN_ed_ll.^2,1),4)));
t_sd = sqrt(squeeze(mean(mean(t_ed_mask_pos.*t_ed_ll.^2,1),4)));
vt_ed = squeeze(mean(mean(t_ed_mask_pos.*vN_ed_ll.*t_ed_ll,1),4));
