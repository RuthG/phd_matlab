...
vNt_ed_pav = mean(vNt_ed_ll(:,:,blev(xn):tlev(xn)-1,:),3);
...
vNt_ed_pav_tneg = squeeze(mean(vNt_ll_tneg(:,:,blev(xn):tlev(xn)-1,:) ,3));
vNt_ed_pav_tpos = squeeze(mean(vNt_ll_tpos(:,:,blev(xn):tlev(xn)-1,:) ,3));
