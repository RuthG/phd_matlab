...
function [vt_ed_tzav,vt_ed_tzav_neg,vt_ed_tzav_pos,vt_ed_sdev,vt_ed_sdev_neg,vt_ed_sdev_pos] = vt_paper_plot_fun_trop(run)
...
vNt_ed_pav = mean(vNt_ed_ll(:,:,blev(xn)+1:tlev(xn),:),3);
...
vNt_ed_pav_tneg = squeeze(mean(vNt_ll_tneg(:,:,blev(xn)+1:tlev(xn),:) ,3));
vNt_ed_pav_tpos = squeeze(mean(vNt_ll_tpos(:,:,blev(xn)+1:tlev(xn),:) ,3));
