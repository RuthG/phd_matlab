...
load(['/project/rg312/mat_files/snapshot_data_final/run_050/t_050_' num2str(j) '.mat'])
t_050_ll = cube2latlon(xc,yc,t_050,xi,yi);
clear('t_050')

[a,b,dtdp_050] = gradient(t_050_ll,-4000);
...
dtdz_050 = g.*rho_050_ll.*dtdp_050;
...
stab_diff(:,:,:) = (dtdz_050(:,:,13,:) - gamma_s_050(:,:,13,:)).*1000;
...
stab_hist_data_midlats(:,j) = sum(stab_counts_cos(x050n-7:x050n+7,:),1) + sum(stab_counts_cos(x050s-7:x050s+7,:),1);

stab_hist_data_midlatstight(:,j) = sum(stab_counts_cos(x050n,:),1) + sum(stab_counts_cos(x050s,:),1);
...
save('/project/rg312/mat_files/snapshot_data_final/run_050/stab_hist_data_050.mat', 'xbins', 'stab_hist_data_full', 'stab_hist_data_midlats', 'stab_hist_data_midlatstight', 'stab_hist_data_tropics')
