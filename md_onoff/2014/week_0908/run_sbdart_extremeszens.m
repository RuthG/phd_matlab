...
%approximate extreme zenith angles and positions

%for delta = 23.26: cos(szen) = sin(lat)sin(delta) + cos(lat)cos(delta)cos(hour)
%so at equator szen=23.26, at pole 66.74
...
szen = acos(sin(yi(i).*pi./180).*sin(23.26.*pi./180) + cos(yi(i).*pi./180).*cos(23.26.*pi./180)).*180./pi;
...
file = '';
 ...
 formatSpec = '  sza =  %3.1f\n';
    fprintf(fileID,formatSpec,szen);
...
command = ['mv OUTPUT sw_output_files_szen/OUTPUT_wv_' int2str(i)]
	system(command)
...
command = ['mv OUTPUT lw_output_files_szen/OUTPUT_wv_l_' int2str(i)]
	system(command)
...
command = ['mv OUTPUT lw_output_files_szen/OUTPUT_wv_u_' int2str(i)]
	system(command)
...
command = ['mv OUTPUT win_output_files_szen/OUTPUT_wv_' int2str(i)]
	system(command)
