...
a=system('find mape_sensa_data.mat');
...
save('mape_sensa_data.mat')
else
load('mape_sensa_data.mat')
end
...
%MAPE!!!!!!! YAY!!!!!
mape_d = sum(integrand_d(4:level_d-1)).*dsigma.*prefactor;
mape_h = sum(integrand_h(4:level_h-1)).*dsigma.*prefactor;
mape_f = sum(integrand_f(4:level_f-1)).*dsigma.*prefactor;
