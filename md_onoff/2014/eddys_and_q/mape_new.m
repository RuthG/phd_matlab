...
a=system('find mape_new_data.mat');
...
save('mape_new_data.mat')
else
load('mape_new_data.mat')
end
...
temp_q_tzav(:,j)=theta_q_tzav(:,j).*convthetatoT(j,1);
end
...
min_no_q = find(abs(ylat) == barzon_lats_q(1)-1);
max_no_q = find(abs(ylat) == barzon_lats_q(2)+1);
else
min_no_q = find(abs(ylat) == barzon_lats_q(1));
max_no_q = find(abs(ylat) == barzon_lats_q(2));
end
...
thsq_q_bzav = (mean(thsq_q_zav(max_no_q(1):min_no_q(1),:),1) + mean(thsq_q_zav(min_no_q(2):max_no_q(2),:),1))./2;
...
theta_q_bzav = (mean(theta_q_tzav(max_no_q(1):min_no_q(1),:),1) + mean(theta_q_tzav(min_no_q(2):max_no_q(2),:),1))./2;

theta_d_bzavsq = theta_d_bzav.^2;
theta_h_bzavsq = theta_h_bzav.^2;
theta_f_bzavsq = theta_f_bzav.^2;
theta_q_bzavsq = theta_q_bzav.^2;
...
thsq_q_ed = thsq_q_bzav - theta_q_bzavsq;
...
[dthetadp_q,junk] = gradient(theta_q_tzav,-4000.);
...
dthetadp_q_bzav = (mean(dthetadp_q(max_no_q(1):min_no_q(1),:),1) + mean(dthetadp_q(min_no_q(2):max_no_q(2),:),1))./2;
...
gamma_q = -2./(7.*rC.*dthetadp_q_bzav');
...
integrand_q = gamma_q.*(rC./10^5).^(2./7).*thsq_q_ed';
...
mape_q = sum(integrand_q(3:level_q)).*dsigma.*prefactor;
save('mape_new_data.mat')
return
