...
htrt_th_vint = sum(htrt_th_mean,3).*intfac;
difht_th_vint = sum(difht_th_mean,3).*intfac;
radht_th_vint = sum(radht_th_mean,3).*intfac;
cndht_th_vint = sum(cndht_th_mean,3).*intfac;
cnvht_th_vint = sum(cnvht_th_mean,3).*intfac;
...
wthll_th=cube2latlon(xc,yc,wth_th_mean,xi,yi);
thetall_th=cube2latlon(xc,yc,theta_th_mean,xi,yi);
vNll_th=cube2latlon(xc,yc,vN_th_mean,xi,yi);
htrtll_th=cube2latlon(xc,yc,htrt_th_vint,xi,yi);
difhtll_th=cube2latlon(xc,yc,difht_th_vint,xi,yi);
radhtll_th=cube2latlon(xc,yc,radht_th_vint,xi,yi);
cnvhtll_th=cube2latlon(xc,yc,cnvht_th_vint,xi,yi);
cndhtll_th=cube2latlon(xc,yc,cndht_th_vint,xi,yi);

vNthll_th_zav(:,:) = mean(vNthll_th,1);
wthll_th_zav(:,:) = mean(wthll_th,1);
...
vNth_th_wt = vNthll_th_zav.*cosphi;
vNth_zon_th_wt = vNthll_zon_th_zav.*cosphi;
...
delwth_th = (wthll_th_zav(:,1) - wthll_th_zav(:,25)).*cp./g;
...
[moo,vNth_zon_th_conv_s] = gradient(vNth_zon_th_wt,pi./90);
...
[moo,vNth_th_conv_s] = gradient(vNth_th_wt,pi./90);
...
vNth_th_conv = -1.*vNth_th_conv_s./cosphi./a;
vNth_zon_th_conv = -1.*vNth_zon_th_conv_s./cosphi./a;
...
vNth_th_vint = intfac.*sum(vNth_th_conv,2);
...
vNth_zon_th_vint = intfac.*sum(vNth_zon_th_conv,2);
...
axes('position',[pos1x pos1y xSize_sub ySize_sub])
plot(yi,htrtll_th_zav,'k')
hold on
ylabel('Vertically integrated heating rate, Wm^{-2}')
title('0xwv')
plot(yi,-vNth_th_vint,'c')
plot(yi,-vNth_th_vint-htrtll_th_zav','r')
ylim([-150 550])
xlim([-90 90])

print('-dpdf','vertheat_checkth.pdf')
...
ylabel('Vertically integrated heating rate, Wm^{-2}')
title('tropheat')
plot(yi,cndhtll_th_zav+cnvhtll_th_zav)
plot(yi,radhtll_th_zav,'g')
plot(yi,difhtll_th_zav,'r')
%plot(yi,-vNth_th_vint,'c')
%plot(yi,-vNth_ed_th_vint,'c')
%plot(yi,-vNth_zon_th_vint,'c--')
ylim([-150 650])
xlim([-90 90])
h=legend('Q_{diabatic}','Q_{latent}','Q_{radiative}','Q_{sensible}'); %,'v''\theta'' Divergence');
%set(h,'Interpreter','latex')
