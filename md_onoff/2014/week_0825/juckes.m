...
theta_th(:,:,:,i)=dyn(:,:,:,J);
for j=1:size(rC)
t_th(:,:,j,i)=theta_th(:,:,j).*convthetatoT(j,1);
end
...
t_th_hzav(i,:) = (t_th_zav(45+i,:) + t_th_zav(46-i,:))./2;
q_th_hzav(i,:) = (q_th_zav(45+i,:) + q_th_zav(46-i,:))./2;
...
teq_th_hzav = t_th_hzav + L./cp .* q_th_hzav;
...
thetaeq_th_hzav(:,j)=teq_th_hzav(:,j)./convthetatoT(j,1);
end
...
dTdz_th = rho_th.*g.*gradient(t_th_hzav,4000);
...
trop_th = max(find(dTdz_th(20,:).*1000 <= -2));
...
delthetaeqz(5) = thetaeq_th_hzav(20,trop_th) - thetaeq_th_hzav(20,1);
...
delty(5) = t_th_hzav(10,13) - t_th_hzav(30,13);
