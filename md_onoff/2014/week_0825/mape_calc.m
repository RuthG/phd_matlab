...
theta_th_hem(:,:,1) = theta_th_zav(46:90,:);
...
thetasq_th_hem(:,:,1) = thetasq_th_zav(46:90,:);
for i=1:45
...
theta_th_hem(i,:,2) = theta_th_zav(46-i,:);
...
thetasq_th_hem(i,:,2) = thetasq_th_zav(46-i,:);
end
...
dthetadp_th = gradient(theta_th_hem,-4000);
...
dthetadp_th_weight(i,:,:) = cos(barzon_reg_th(i).*pi./180) .* dthetadp_th(start_th+i,:,:) ;
theta_th_weight(i,:,:) = cos(barzon_reg_th(i).*pi./180) .* theta_th_hem(start_th+i,:,:) ;
thetasq_th_weight(i,:,:) = cos(barzon_reg_th(i).*pi./180) .* thetasq_th_hem(start_th+i,:,:) ;
...
dthetadp_th_av = mean(dthetadp_th(13:28,:,:),1);
theta_th_av = mean(theta_th_hem(13:28,:,:),1);
thetasq_th_av = mean(thetasq_th_hem(13:28,:,:),1);

dthetadp_th_bzav(:,:) = sum(dthetadp_th_weight,1)./sum(cosy_th) ;
theta_th_bzav(:,:) = sum(theta_th_weight,1)./sum(cosy_th) ;
thetasq_th_bzav(:,:) = sum(thetasq_th_weight,1)./sum(cosy_th) ;
...
for k=1:25
gamma_th(k,:) = -kappa./rC(k) .* 1./dthetadp_th_bzav(k,:);
...
plot(gamma_th_av,rC,'m')
print('-dpng','gamma_plots.png')

theta_var_th = mean(thetasq_th_bzav - theta_th_bzav.^2,2);
...
plot(theta_var_th,rC,'m')
print('-dpng','theta_var.png')

%calculate integrand
for i=1:2
mape_integrand_th(:,i) = (rC./p0).^kappa .*gamma_th(:,i) .* (thetasq_th_bzav(:,i) - theta_th_bzav(:,i).^2) ;
...
mape_integrand_th_av = mean(mape_integrand_th,2);
...
plot(mape_integrand_th_av,rC,'m')
...
%integrate
mape_th = prefac .* sum(mape_integrand_th(3:level_th,:),1) .* 4000./p0;
...
%save
name = ['mape_out.mat'];
save(name, 'mape_out_d','mape_out_q','mape_out_h','mape_out_w','mape_out_th')
