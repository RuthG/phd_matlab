...
temp_th_tav(:,:,j)=theta_th_tav(:,:,j).*convthetatoT(j,1);
end
...
vNtll_th=cube2latlon(xc,yc,vNt_th_tav,xi,yi);
thetall_th=cube2latlon(xc,yc,theta_th_tav,xi,yi);
templl_th=cube2latlon(xc,yc,temp_th_tav,xi,yi);
...
rhoair_th(:,i) = rC(i)./(287.058.*temp_th_zav(:,i));
end
...
dtempdp_th = gradient(temp_th_zav,-4000.);
dtempdz_th = -1.*rhoair_th.*g.*dtempdp_th.*1000;
...
trop_th(i) = rC(min(find((dtempdz_th(i+1,:) >= -2))));
end
...
sigma_t_max_th = max(trop_th);
level_th = find(rC == sigma_t_max_th);
...
vN_th_zav_t_zav = vN_th_zav.*theta_th_zav;
vt_ed_th_zav = vNt_th_zav - vN_th_zav_t_zav;
...
barzon_a_th(i,:) = vt_ed_th_zav(i,1:level_th).*cos(yi(i).*pi./180);
end
...
barzon_b_th = dsigma.*sum(barzon_a_th,2);
...
barzon_c_th(i) = (barzon_b_th(91-i) - barzon_b_th(i))./2;
end
...
barzon_centre_th = find(barzon_c_th == max(barzon_c_th));
...
barzon_lats_th(1) = abs(yi(barzon_centre_th)) - 15;
barzon_lats_th(2) = abs(yi(barzon_centre_th)) + 15;
...
plot_min_th = barzon_lats_th(1)-1;
plot_max_th = barzon_lats_th(2)+1;
else
plot_min_th = barzon_lats_th(1);
plot_max_th = barzon_lats_th(2);
end
...
name = ['bc_box_th.mat'];
save(name,'barzon_lats_th','level_th')
