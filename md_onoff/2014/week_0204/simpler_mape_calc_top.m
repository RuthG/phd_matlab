...
dthetadz_bzav = (sum(dthetadz(max_no(1):min_no(1),:),1)./weight_sum + sum(dthetadz(min_no(2):max_no(2),:),1)./weight_sum)./2;
dthetadp_bzav = (sum(dthetadp(max_no(1):min_no(1),:),1)./weight_sum + sum(dthetadp(min_no(2):max_no(2),:),1)./weight_sum)./2;
dthetady_bzav = (sum(-dthetady(max_no(1):min_no(1),:),1)./weight_sum + sum(dthetady(min_no(2):max_no(2),:),1)./weight_sum)./2;
theta_bzav = (sum(theta_tzav(max_no(1):min_no(1),:),1)./weight_sum + sum(theta_tzav(min_no(2):max_no(2),:),1)./weight_sum)./2;
...
simp_mape_vert = integrand.*4000.*merid_width.^2;

plot(simp_mape_vert,rC)
set(gca,'YDir','reverse')
xlabel('MAPE, J/m^2')
ylabel('Pressure, Pa')
name = ['simp_mape_top_' dir_label '.png'];
print('-dpng',name)

name = ['simp_mape_top_' dir_label '.mat'];
save(name)
