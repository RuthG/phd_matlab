...
vt_bins = -162: 3 : 264;
vt_bins_c = -160.5: 3 : 264;

peak_dist_000 = zeros(142,1);
peak_dist_010 = zeros(142,1);
peak_dist_025 = zeros(142,1);
peak_dist_050 = zeros(142,1);
peak_dist_075 = zeros(142,1);
peak_dist_100 = zeros(142,1);
...
for i=1:15
  ...
  for k=1:142
  peak_dist_000(k) = peak_dist_000(k) + sum( a_000 >= vt_bins(k) & a_000 < vt_bins(k+1));
  peak_dist_010(k) = peak_dist_010(k) + sum( a_010 >= vt_bins(k) & a_010 < vt_bins(k+1));
  peak_dist_025(k) = peak_dist_025(k) + sum( a_025 >= vt_bins(k) & a_025 < vt_bins(k+1));
  peak_dist_050(k) = peak_dist_050(k) + sum( a_050 >= vt_bins(k) & a_050 < vt_bins(k+1));
  peak_dist_075(k) = peak_dist_075(k) + sum( a_075 >= vt_bins(k) & a_075 < vt_bins(k+1));
  peak_dist_100(k) = peak_dist_100(k) + sum( a_100 >= vt_bins(k) & a_100 < vt_bins(k+1));
  end
...
save('/project/rg312/mat_files/vt_extreme_data.mat','vt_thresh_000','vt_thresh_010','vt_thresh_025','vt_thresh_050','vt_thresh_075','vt_thresh_100' ...
,'peaks_000','peaks_010','peaks_025','peaks_050','peaks_075','peaks_100')


save('/project/rg312/mat_files/vt_ed_data.mat','vt_ed_000','vt_ed_010','vt_ed_025','vt_ed_050','vt_ed_075','vt_ed_100')
