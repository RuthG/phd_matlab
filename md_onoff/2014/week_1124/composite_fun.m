...
function [comp_data,check] = composite_fun(vt_peaks,vt_thresh,var_n,var_s,filename);
...
comp_data(:,:,vtindex) = NaN;
  ...
  %for the 'normal' case, where the upper bound is bigger than the lower bound, fill in cross sec from centre outwards
      ...
      comp_data(18:18+uplim,:,vtindex) = squeeze(var(positions(k):upper,i,:,j));
      comp_data(18-lowlim:18,:,vtindex) = squeeze(var(lower:positions(k),i,:,j));
...
%and split the lower portion into the half that is from 1 to the peak centre
      comp_data(19-positions(k):18,:,vtindex) = squeeze(var(1:positions(k),i,:,j));
%and the half that is from the lower bound to 180
      comp_data(18-lowlim:18-positions(k),:,vtindex) = squeeze(var(lower:180,i,:,j));
 ...
 %and split the upper portion into the half that is from the peak centre to 180
      comp_data(18:18+(180 - positions(k)),:,vtindex) = squeeze(var(positions(k):180,i,:,j));
 %and the half that is from 1 to the upper limit
      comp_data(19+(180-positions(k)):18+uplim,:,vtindex) = squeeze(var(1:upper,i,:,j));
