%function [UVcs,U2tot,U2trans,V2tot,V2trans,errFlag]=calc2ndmom(u,v,usq,vsq,uv,cosalpha,sinalpha);
...
hs=rdmds([rDir,'hFacS']);
ar=rdmds([rDir,'RAC']);
cosalphai=rdmds([rDir,'AngleCS']);
sinalphai=rdmds([rDir,'AngleSN']);
...
sinalpha=reshape(sinalphai,[nPg 1]);
cosalpha=reshape(cosalphai,[nPg 1]);
%
% Read cos and sin of rotation angle if not provided on input
if nargin == 5
 Rac='/disk1/MITgcm/verification/global_ocean.cs32x15/input/';
...
%
for k=1:nz;
checkpoint(:,k) = ( UV(:,k) + (v2j(:,k) - u2i(:,k)).*cosalpha.*sinalpha ) .*(cosalpha.^2 - sinalpha.^2);
checkpointb(:,k) = (1 - 4.*cosalpha.*cosalpha.*sinalpha.*sinalpha);
UVcs(:,k) = ( UV(:,k) + (v2j(:,k) - u2i(:,k)).*cosalpha.*sinalpha ) .*(cosalpha.^2 - sinalpha.^2) ...
               ./ (1 - 4.*cosalpha.*cosalpha.*sinalpha.*sinalpha);
