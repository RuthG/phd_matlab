...
for i=1:15
  ...
  for k=1:600
  peak_dist_000(k) = peak_dist_000(k) + sum( vt_ed_000n(:,i,4,j) >= vt_bins(k) & vt_ed_000n(:,i,4,j) < vt_bins(k+1));
  peak_dist_000(k) = peak_dist_000(k) + sum( vt_ed_000s(:,i,4,j) >= vt_bins(k) & vt_ed_000s(:,i,4,j) < vt_bins(k+1));
  peak_dist_010(k) = peak_dist_010(k) + sum( vt_ed_010n(:,i,4,j) >= vt_bins(k) & vt_ed_010n(:,i,4,j) < vt_bins(k+1));
  peak_dist_010(k) = peak_dist_010(k) + sum( vt_ed_010s(:,i,4,j) >= vt_bins(k) & vt_ed_010s(:,i,4,j) < vt_bins(k+1));
  peak_dist_025(k) = peak_dist_025(k) + sum( vt_ed_025n(:,i,4,j) >= vt_bins(k) & vt_ed_025n(:,i,4,j) < vt_bins(k+1));
  peak_dist_025(k) = peak_dist_025(k) + sum( vt_ed_025s(:,i,4,j) >= vt_bins(k) & vt_ed_025s(:,i,4,j) < vt_bins(k+1));
  peak_dist_050(k) = peak_dist_050(k) + sum( vt_ed_050n(:,i,4,j) >= vt_bins(k) & vt_ed_050n(:,i,4,j) < vt_bins(k+1));
  peak_dist_050(k) = peak_dist_050(k) + sum( vt_ed_050s(:,i,4,j) >= vt_bins(k) & vt_ed_050s(:,i,4,j) < vt_bins(k+1));
  peak_dist_075(k) = peak_dist_075(k) + sum( vt_ed_075n(:,i,4,j) >= vt_bins(k) & vt_ed_075n(:,i,4,j) < vt_bins(k+1));
  peak_dist_075(k) = peak_dist_075(k) + sum( vt_ed_075s(:,i,4,j) >= vt_bins(k) & vt_ed_075s(:,i,4,j) < vt_bins(k+1));
  peak_dist_100(k) = peak_dist_100(k) + sum( vt_ed_100n(:,i,4,j) >= vt_bins(k) & vt_ed_100n(:,i,4,j) < vt_bins(k+1));
  peak_dist_100(k) = peak_dist_100(k) + sum( vt_ed_100s(:,i,4,j) >= vt_bins(k) & vt_ed_100s(:,i,4,j) < vt_bins(k+1));
  end
...
figure
plot(vt_bins_c, peak_dist_000,'r')
xlim([-500 500])
set(gca,'FontSize',15);
hold on
plot(vt_bins_c, peak_dist_010,'c')
plot(vt_bins_c, peak_dist_025,'m')
plot(vt_bins_c, peak_dist_050,'g')
plot(vt_bins_c, peak_dist_075,'b')
plot(vt_bins_c, peak_dist_100,'k')
xlabel('V''T'' bin, K m/s','FontSize',15)
ylabel('Number of occurences','FontSize',15)
title('PDF of V''T'' values','FontSize',15)
legend('0.0es_{0}','0.1es_{0}','0.25es_{0}','0.5es_{0}','0.75es_{0}','1.0es_{0}','FontSize',15)
print('-dpng','vtpdf.png')
...
for i=1:15
  ...
  [a_000n,b_000n] = findpeak_prom(vt_ed_000n(:,i,4,j),0);
  [a_000s,b_000s] = findpeak_prom(vt_ed_000s(:,i,4,j),0);
  [a_010n,b_010n] = findpeak_prom(vt_ed_010n(:,i,4,j),0);
  [a_010s,b_010s] = findpeak_prom(vt_ed_010s(:,i,4,j),0);
  [a_025n,b_025n] = findpeak_prom(vt_ed_025n(:,i,4,j),0);
  [a_025s,b_025s] = findpeak_prom(vt_ed_025s(:,i,4,j),0);
  [a_050n,b_050n] = findpeak_prom(vt_ed_050n(:,i,4,j),0);
  [a_050s,b_050s] = findpeak_prom(vt_ed_050s(:,i,4,j),0);
  [a_075n,b_075n] = findpeak_prom(vt_ed_075n(:,i,4,j),0);
  [a_075s,b_075s] = findpeak_prom(vt_ed_075s(:,i,4,j),0);
  [a_100n,b_100n] = findpeak_prom(vt_ed_100n(:,i,4,j),0);
  [a_100s,b_100s] = findpeak_prom(vt_ed_100s(:,i,4,j),0);
...
save('/project/rg312/mat_files/snapshot_data/vt_extreme_data_noprom.mat','vt_thresh_000','vt_thresh_010','vt_thresh_025','vt_thresh_050','vt_thresh_075','vt_thresh_100' ...
