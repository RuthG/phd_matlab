...
[dif_m_zav,mskzon,ylat,areazon]=calcZonalAvgCube(dif_m,ny,yc,ar,hc);
...
uv_grad_mean = mean(uv_grad,3);
duEdt_mean = mean(duEdt_zav,3);
fvN_mean = mean(fvN_zav,3);

for i=1:90
fvN_mean_weighted(i,:) = fvN_mean(i,:).*cos(ylat(i).*pi./180);
duEdt_mean_weighted(i,:) = duEdt_mean(i,:).*cos(ylat(i).*pi./180);
end
...
duEdt_zav_vint(:,:) = sum(duEdt_zav.*4000,2)./9.81;

dif_m_vint(:,:) = sum(dif_m_zav,2);

for i=1:90
dif_m_vint_weighted(i,:) = cos(ylat(i).*pi./180).^2.*dif_m_vint(i,:);
end
