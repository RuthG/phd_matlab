...
if wv_exponent == 0.
  for k= 1:n
    del_tau = (ir_tau_co2 + ir_tau_wv1.*q_in(:,k) + ir_tau_wv2.*q_in(:,k).^2).*(p_half(k+1) - p_half(k))./p_half(n+1);
...
window_surf = -0.0967.*t_surf.^2./100./100 + 0.6516.*t_surf./100 -0.7089;
...
save('radmod_sw_winnp.mat','solar_down','flux_sw','tdt_sw','solar_tau','dtrans_sol') 

save('radmod_lw_winnp.mat','down','down_win','up','up_win','net','net_win','tdt_lw','tdt_win','dtrans','dtrans_win','b','b_win','q_in','t_in','t_surf','b_surf')
