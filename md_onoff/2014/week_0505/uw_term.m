...
[uw_zav,mskzon,ylat,areazon]=calcZonalAvgCube(uw_ctrl,ny,yc,ar,hc);
...
uw_tzav = mean(uw_zav,3);

%find duwdp
duwdp_tzav = gradient(uw_tzav,-4000);
...
for i=1:90
uv_ed_tzav_weighted(i,:) = uv_ed_tzav(i,:).*cos(ylat(i).*pi./180).^0 ;
uv_zon_tzav_weighted(i,:) = cos(ylat(i).*pi./180).^0.*uv_zon_tzav(i,:);
uv_tzav_weighted(i,:) = cos(ylat(i).*pi./180).^0.*uv_tzav(i,:);
...
for i=1:90
uv_ed_grad(i,:) = -1.*uv_ed_grad_s(i,:)./(a.*cos(ylat(i).*pi./180).^0);
uv_grad(i,:) = -1.*uv_grad_s(i,:)./(a.*cos(ylat(i).*pi./180).^0);
uv_zon_grad(i,:) = -1.*uv_zon_grad_s(i,:)./(a.*cos(ylat(i).*pi./180).^0);
end

for i=1:90
%fvN_mean_weighted(i,:) = fvN_mean(i,:).*cos(ylat(i).*pi./180);
duEdt_tzav_w(i,:) = duEdt_tzav(i,:).*sin(ylat(i).*pi./180).^2;
