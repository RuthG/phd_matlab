...
load('lw_sbdart_szen.mat')
load('sw_sbdart_szen.mat')
load('radmod_sw_szen.mat')
load('radmod_lw_szen.mat')

for i=1:25
trans_sw(:,i) = fxdn_sw_half(:,i+1)./fxdn_sw_half(:,1);
dtrans_sw(:,i) = fxdn_sw_half(:,i+1)./fxdn_sw_half(:,i);
dtrans_sol_recalc(:,i) = solar_down(:,i+1)./solar_down(:,i);
end
...
[C,h] = contour(yi,p,dtrans_sol_recalc',v,'k');
clabel(C,h);
set(gca,'YDir','reverse')
xlabel('Latitude')
title('SW transmission (RadMod)')

print('-dpdf','sw_flux_trans_szen.pdf')
...
print('-dpdf','lw_fluxwin_szen.pdf')
