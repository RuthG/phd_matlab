...
function [vNH_ed_out ,vNH_ed_warm_out ,vNH_ed_cold_out ,vND_ed_out ,vND_ed_warm_out ,vND_ed_cold_out] = vH_PW_fun_trop(run)
...
vNH_ed_pzint(:,:,year) = 2.*pi.*a.*cos(yi(xn).*pi./180).*squeeze(mean(mean(vNH_ed(:,:,1:tlev(xn),:),1),3))./g;
vNH_ed_warm_pzint(:,:,year) = 2.*pi.*a.*cos(yi(xn).*pi./180).*squeeze(mean(mean(vNH_ed_warm(:,:,1:tlev(xn),:),1),3))./g;
vNH_ed_cold_pzint(:,:,year) = 2.*pi.*a.*cos(yi(xn).*pi./180).*squeeze(mean(mean(vNH_ed_cold(:,:,1:tlev(xn),:),1),3))./g;

vND_ed_pzint(:,:,year) = 2.*pi.*a.*cos(yi(xn).*pi./180).*squeeze(mean(mean(vND_ed(:,:,1:tlev(xn),:),1),3))./g;
vND_ed_warm_pzint(:,:,year) = 2.*pi.*a.*cos(yi(xn).*pi./180).*squeeze(mean(mean(vND_ed_warm(:,:,1:tlev(xn),:),1),3))./g;
vND_ed_cold_pzint(:,:,year) = 2.*pi.*a.*cos(yi(xn).*pi./180).*squeeze(mean(mean(vND_ed_cold(:,:,1:tlev(xn),:),1),3))./g;
%end
