%mit_lw.m
%Run MIT's longwave radiation scheme in matlab, test the effects of changing the coefficients.

load('sbdart_lw.mat')

ir_tau_co2 	= 1. ;0.8678;
ir_tau_wv 	= 1000. ;1.9979e+3;
q 		= q_mit;
p		= 2000:4000:98000;
...
for k = 1:25
    del_tau(k)    = ( ir_tau_co2 + ir_tau_wv * q(k) ) * ( p_half(k+1)-p_half(k) ) / p_half(26);
...
for k = 1:25
  tdt_rad(k) = (net(k+1) - net(k)).*grav.*spd/(cp_air.*(p_half(k+1)-p_half(k))) ;
end
