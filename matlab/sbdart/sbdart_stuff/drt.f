...
c-----------------------------------------------------------------------
c **********************************************************************
c opening standard file for listing of model run output/diagnostics
c Edited 19/04/04: H. Brindley
c **********************************************************************
      open(unit=30,file='OUTPUT',status='new')
...
c assign user radiance angles
      ...
      if(radcalc) call vuangles(nphi,phi,nzen,uzen) ! find viewing angles
...
c.......................................................................
      contains 
c.......................................................................
      subroutine vuangles(nphi,phi,nzen,uzen) ! internal procedure

c     modifies values of nphi, phi, nzen, uzen

      real(kr) :: p1,p2,z1,z2,xxx,zipp,phi(:),uzen(:)
      integer :: i,ii,nphi,nzen
...
c
     ...
     &     call errmsg(17,'CHKIN--CORINT=t, but flux output selected')
      ...
      if(.not. any(iout.eq.(/0,1,5,6,7,10,11,20,21,22,23/))) then
        call ck(kill,'iout','[0,1,5,6,7,10,11,20,21,22,23]')
        print *,'iout',iout
      endif
...
c **********************************************************************
c This is where the outputting of info starts. Have changed the write(*)
c statements in the original code to write(30) after opening file 30
c as a standard output file (OUTPUT) in the main code
c Edited 19/04/04: H. Brindley 
c **********************************************************************
...
c  
 ...
 1000 format(10es12.4)
      ...
      if(iout.eq.1 .or. iout.eq.5 .or. iout.eq.6) then
        ...
        if(kd.eq.nk.and.ib.eq.1) then
          ...
          write(30,'(f12.8,f9.5,6es12.4)') wl,weq/wfull,
     ...
     &       real(botdn/weq),real(botup/weq),real(botdir/weq)
        ...
        if(iout.eq.5 .or. iout.eq.6 ) then
          ...
          if(kd.eq.nk.and.ib.eq.1) then
            if(weq.eq.0.) weq=1.e-30
            write(30,'(3i4)')       nphi,nzen
            write(30,1000) (real(phi(j)),j=1,nphi)
            write(30,1000) (real(uzen(j)),j=1,nzen)
            do i=nzen,1,-1
              write(30,1000) (real(uurs(i,k)/weq),k=1,nphi)
            enddo
          endif
        endif
      endif
 ...
 10     continue
      ...
      if(iout.eq.7.and.kd.eq.nk.and.ib.eq.1) then
        write(30,'(//,f12.8)') wl
        write(30,'(/(10es11.3))') (z(im),im=nz,1,-1)
        write(30,'(/(10es11.3))') (real(fxdir(i)),i=1,nz)
        write(30,'(/(10es11.3))') (real(fxdn(i)-fxdir(i)),i=1,nz)
        write(30,'(/(10es11.3))') (real(fxdn(i)),i=1,nz)
        write(30,'(/(10es11.3))') (real(fxup(i)),i=1,nz)
      endif
...
c       fxdn(nz) corresponds to total downward flux at surface
        ...
        do 10 i=1,nz
          ...
          zm=zz
          pm=pp
          write(30,1000) zz,pp,real(fxdn(i)),real(fxup(i)),
     &         real(fxdir(i)),real(dfdz),real(heat)
 10     continue
      endif

      if(iout.eq.10.or.iout.eq.20.or.iout.eq.21.or.iout.eq.23)
     &   write(30,'(3f11.4,6es12.4)') wlinf,wlsup,phidw,
     &     real(topdn),real(topup),real(topdir),
     &     real(botdn),real(botup),real(botdir)

      if(iout.eq.20.or.iout.eq.21.or.iout.eq.23) then
        write(30,'(3i4)')       nphi,nzen
        write(30,1000) (phi(j),j=1,nphi)
        write(30,1000) (uzen(j),j=1,nzen)
        do 20 i=nzen,1,-1
          write(30,'(20es12.4)' ) (real(uurs(i,k)),k=1,nphi)
 20     continue
      endif

      if(iout.eq.22) then
        write(30,'(3i4,es12.4)') nphi,nzen,nz,phidw
        write(30,1000) (phi(i),i=1,nphi)
        write(30,1000) (uzen(j),j=1,nzen)
        write(30,1000) (z(k),k=nz,1,-1)
        write(30,1000) (real(fxdn(k)),k=1,nz)
        write(30,1000) (real(fxup(k)),k=1,nz)
        write(30,1000) (real(fxdir(k)),k=1,nz)
        write(30,1000)
...
c     &     10000./wvnmlo-10000./wvnmhi
      ...
      contains
c.......................................................................
      function wlwn(x)          ! internal procedure
...
c.......................................................................
      function wlln(x)          ! internal procedure
